...
def check_password(ip, password):
    # Get the admin challenge index for this IP
...
def get_client_ip():
    ...
    print(f"[DEBUG] Client IP detected: {ip}")
    return ip
...
def check():
    ...
    try:
        ...
        error, status = validate_radius(radius_miles, ip)
        if error:
...
def appeal():
    ...
    save_bannage()
    timetime = format_ban_time(cooldown_remaining)
    if user["strikes"] >= 25600 and user.get("cooldown_until", 0) > now:
        prev = user["strikes"]
        user["strikes"] = min(int(prev * 1.15) + 5, MAX_STRIKES)
        save_bannage()
        raise Forced404    
...
def ban_ip():
    ...
    now = time.time()
    cooldown_hours = 24  # Or however long you want to ban for, can be a param too
    cooldown_until = now + cooldown_hours * 3600

    # Update or create the ban entry for the target IP
    ...
    # Format cooldown time nicely for flash message
